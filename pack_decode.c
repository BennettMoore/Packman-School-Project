...
/**
 * @brief Helps decode_bits traverse through the huffman tree
 */
void decoder_helper(Tree_node root, uchar * instructions, uint num_bits, 
		FILE * output){
	...
	while(instruction_index < num_bits){
		...
		while(curr_node->left != NULL && curr_node->right != NULL){
			...
			else if(instructions[instruction_index] == NUL){
				break; //End of instructions reached
			}
			else{ //Illegal symbol
				errno = EINVAL;
				report_error("pack_decode:decoder_helper", 
					__LINE__, 
					(char * )&instructions
					[instruction_index], 
					" Encountered instruction that was not"
					" '0' or '1'");
				return;
			}
			instruction_index++;
		...
		if(fwrite(&curr_node->sym, sizeof(uchar), 1, output) != 1){
			
			//fwrite failed
			errno = EINTR;
			report_error("pack_decode:decoder_helper", __LINE__, 
				"fwrite", " Could not write symbol to file");
			return;
		}
	...
	if(curr_node != root){ //Fragmented codeword(s) found
		errno = EILSEQ;
		report_error("pack_decode:decoder_helper", __LINE__, "root", 
			" Instructions did not correctly line up " 
			"to the provided huffman tree");
	}
}
 ...
 */
void decode_bits(Tree_node root, uint num_bits, uint * bit_array, 
		FILE * output){
...
void parse_file(FILE * input, FILE * output){
	...
	if(root == NULL){ //read_tree failed
		...
		report_error("pack_decode.c:parse_file", __LINE__, 
			"read_tree", 
			" Huffman tree could not be created from"
			" encoded file");
	...
	if(fread(&num_bits, sizeof(uint), 1, input) != 1){ //fread failed
		...
		report_error("pack_decode.c:parse_file", __LINE__, 
			"fread", 
			" Number of bits used in the encoding not found");
	...
	uint bit_array[array_size];
	size_t words_read = fread(bit_array, sizeof(uint), array_size, input); 
	
	if(words_read != array_size){ //fread failed
		fprintf(stderr, "pack_decode.c:parse_file:%i:"
			" Read in %lu words of bit_array, supposed to"
			" read in %lu words.\n", __LINE__, words_read, 
			array_size);
	}
	else{
